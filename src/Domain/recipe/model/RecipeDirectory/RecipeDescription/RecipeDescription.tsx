...
const RecipeDescription: FC<Props> = ({errors, register, control, getValues, setValue, currentRecipeCode, recipeDescData, reset, recipe}) => {
    ...
    const {data: compFiller} = compAPI.useFetchAllCompQuery('3000')
    const {data: compWat} = compAPI.useFetchAllCompQuery('1000')
    const {data: compCem} = compAPI.useFetchAllCompQuery('4000')
    const {data: compChem} = compAPI.useFetchAllCompQuery('2000')
